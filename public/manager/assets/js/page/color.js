    ...
    table: {
        __generateDTRow(data){ 
            return [
                ...
                data.name, 
                `<div style="width: 20px; height: 20px; background-color: ${data.hex}"></div>`, 
        ...
        init(){
            var row_table = [ 
                ...
                { title: 'Hex', name: 'name', orderable: true, },  
    ...
    FullTab: {  
        Create: { 
            ...
            getVal(resource){ 
                ...
                var data_color           = $(`${resource}`).find('.data-color').val().replace(noTag, "");  

                // --Required Value 
                if (data_name == '') { required_data.push('Name valid.'); onPushData = false }  
                if (data_color == '') { required_data.push('Color valid.'); onPushData = false } 
        ...
        Update: { 
            setVal(resource, data){
                ...
                $(`${resource}`).find('.data-color').val(data.hex); 
            },
            getVal(resource){ 
                var fd = new FormData();
                var required_data = [];
...
(() => {
    ...
    View.FullTab.onPush("Delete", "#popup-delete", () => {
        ...
        View.FullTab.default();
        IndexView.helper.showToastProcessing('Process', 'Processing');
        Api.Color.Delete($("#popup-delete").find('.data-id').val())
            .done(res => { 
                IndexView.helper.showToastSuccess('Success', 'Success');
                getData();
            })
